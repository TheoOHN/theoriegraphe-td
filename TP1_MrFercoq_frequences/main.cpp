...
class Station
{
    private:
        ...
        std::vector< Station*> m_adjacents;
    ...
    public:
        ...
        std::vector< Station*> get_adjacents()
        {
            return m_adjacents;
        }
        ...
        friend std::ostream& operator<<(std::ostream& out,  Station& s)
        {
...
bool decroissant(Station * s1, Station * s2)
{
    return s1->getDegre() > s2->getDegre();
}

bool station_equal(Station * s1, Station * s2)
{
    return s1->getId() == s2->getId();
}

bool in_vector_station(std::vector<Station*> stations, Station * station)
{
    for (auto elem : stations)
    {
        if (station_equal(elem, station))
            return true;
    }
...
class Reseau
{
    ...
    public:
        ...
        {
            /// A COMPLETER
            std::vector<Station*> stations_copie(m_stations);

            std::vector<std::vector <Station*>> couleurs;
            std::vector<Station*> couleur;

            std::vector<Station*> adjacents;

            do
            {
                for (unsigned int i = 0 ; i < stations_copie.size() ; i++)
                {
                    if (!in_vector_station(adjacents, stations_copie[i]))
                    {
                        couleur.push_back(stations_copie[i]);
                        //std::cout << *stations_copie[i];

                        for (auto elem : stations_copie[i]->get_adjacents())
                        {
                            adjacents.push_back(elem);
                        }

                        stations_copie.erase(stations_copie.begin() + i);
                    }
                }
            ...
            } while(stations_copie.size() > 0);

            int compteur = 0;

            for(auto vecteur_couleur : couleurs)
            {
                compteur++;
                std::cout << std::endl << "COULEUR " << compteur << std::endl << std::endl;

                for (auto couleur : vecteur_couleur)
                {
                    std::cout << *couleur;
                }
            }
        ...
        void attribuerWelshPowell(int nbFrequences)
        {
           /// A COMPLETER
            ...
            std::sort(stations_copie.begin(), stations_copie.end(), decroissant);
...
int main()
{
    try
    {
        ...
        std::cout << reseau;
        reseau.attribuerNaif(reseau.getOrdre());
        reseau.attribuerWelshPowell(reseau.getOrdre());
    }
